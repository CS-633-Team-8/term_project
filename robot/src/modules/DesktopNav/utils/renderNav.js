...
export default function renderNav(groups, { onClick, pathname }) {
    return groups.map((group, index) => (<AkNavigationItemGroup title={group.title} key={pathname + index + (group.title || '')}>
      {group.items.map(item => {
        ...
        return item.external ? (<ExternalNavigationItem key={item.title} href={item.to} icon={icon} text={item.title}/>) : (<RouterNavigationItem isCompact={item.isCompact} key={item.title} href={item.to} icon={icon} onClick={onClick} text={item.title} isSelected={isSelected} pathname={pathname} subNav={item.items}/>);
    })}
