...
//         .filter(match => match.name.includes('navigate-') || match.name === 'loaded');
//     if (navigate && loaded) {
...
//             sendApdex(navigate.name.replace('navigate-', ''), Math.round(entries[0].duration));
//         }
//     }
...
const WrappedLoadable = ({ render, ...rest }) => Loadable({
    ...rest,
